...
public class GyroManager : MonoBehaviour
{
    ...
    private readonly float m_Deadzone = 0.1f;
#elif UNITY_EDITOR
    ...
    private void FixedUpdate()
    {
        ...
        if (playerCharacter != null && playerCharacter.Rb != null)
        {
            ...
            if (gravityForce.magnitude < m_Deadzone)
            {
            ...
            // z축 0일 때 왼쪽을 바라보는게 기본 상태이므로, lookDirection을 바라보게 합니다.
            playerCharacter.transform.DORotate(new Vector3(0, 0, angle), 0.7f); // 오프셋 제거
            ...
            if (gravityDirection.magnitude < m_Deadzone)
            {
            ...
            if (gravityDirection.magnitude > 0.1f) // 입력이 있을 때만 회전
            {
                ...
                float angle = Mathf.Atan2(lookDirection.y, lookDirection.x) * Mathf.Rad2Deg; // angle 변수 선언 추가
                // z축 0일 때 왼쪽을 바라보는게 기본 상태이므로, lookDirection을 바라보게 합니다.
                playerCharacter.transform.DORotate(new Vector3(0, 0, angle), 0.7f); // 오프셋 제거
            }
#endif
